    ...
    <div>
        <Row>
            <Form ref="searchForm" :model="searchForm" :label-width="85">
                ...
                <Row>
                    <Col span="12">
                    <Form-item label="用户：" prop="userNo">
                        <Select v-model="searchForm.userNo" filterable multiple>
                ...
                <Row>
                    <Col span="12">
                    <Form-item label="作业：" prop="homeworkId">
                        <Select v-model="searchForm.homeworkId" filterable multiple>
                            <Option v-for="item in homeworkList" :value="item.value" :key="item.value">
                                {{ item.label }}
                            </Option>
                        </Select>
                    </Form-item>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            getHomeworkIdByTeacher(teacherNo) {
                return new Promise((resolve, reject) => {
                    api_QueryAllHomework(this.homeworkSearchForm).then(response => {
                        ...
                        if (code == 0) {
                            ...
                            for (let i = 0; i < content.length; i++) {
                                this.searchForm.homeworkId.push(content[i].id)
                            }
            ...
            getUserData() {
                return new Promise((resolve, reject) => {
                    api_GetUserOption(this.userSearchForm).then(response => {
                        const code = response.code;
                        const data = response.data;
            ...
            getHomeworkData() {
                return new Promise((resolve, reject) => {
                    api_GetHomeworkOption(this.homeworkSearchForm).then(response => {
                        const code = response.code;
                        const data = response.data;
                        const message = response.message;
                        ...
                        if (code == 0) {
                            ...
                            for (let i = 0; i < content.length; i++) {
                                this.homeworkList.push({
                                    value: content[i].id,
                                    label: content[i].title + '-' + content[i].teacherName
                                })
                            }
                        } else {
                            this.$Modal.error({
