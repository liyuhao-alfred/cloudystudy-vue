			...
			<Col span="6">
				<Card>
					<div v-if="is_video">
						...
						<p>文件大小: {{video_size}}</p>
						<p>识别结果</p>
						<i-circle :size="100" :trail-width="4" :stroke-width="5" :percent="sex_percent" stroke-linecap="square" stroke-color="#ff9900">
							<div class="demo-i-circle-custom">
								<h1>{{sex_count}}</h1>
								<p>性感镜头数</p>
								<span>总占比<i>{{sex_percent+"%"}}</i></span>
							</div>
						</i-circle>
						<i-circle :size="100" :trail-width="4" :stroke-width="5" :percent="porn_percent" stroke-linecap="square" stroke-color="#ed3f14">
							<div class="demo-i-circle-custom">
								<h1>{{porn_count}}</h1>
								<p>涉黄镜头数</p>
								<span>总占比<i>{{porn_percent+"%"}}</i></span>
							</div>
						</i-circle>
					</div>
				</Card>
